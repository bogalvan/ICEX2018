...
void getAnchorCameraWeights(){
   ...
   highLevelCutOff = nodeWeightMean; //.5*SD + nodeWeightMean;
	for (int i =0; i < Nodes.size(); i++){
...
int generateNewNode(int numNodes){
	//cout << "start tree generation \nmap tree size: " << roadMap.size() << "\n";
   //cout << "high weight nodes size: " << highWeightNodes.size() << "\n";
   Anchor curr;
   Anchor newNode = Anchor();
   int nodeIndex = -1;

  	//if odd iteration chose high weight node
   if(iteration % 2 == 0){
   	    //cout << "even iteration choosing from high weight node index: " << highestNodeIndex << "\n";
   		//cout << "highestWeightNodes size: " << highWeightNodes.size() << "\n";	
	   curr = highWeightNodes[highestNodeIndex];
	   highestNodeIndex++;
	   //cout << "even curr index: " << curr.ndex << "\n";
   }else{ //chose a random node in roadmap untill nodes weight is > weightThreshold 
	   do{
	   	nodeIndex = rand() % roadMap.size();
	   	//cout<<"random index: " << nodeIndex <<"\n";
			curr = roadMap.at(nodeIndex);
			//cout << "weight: " << curr.weight << "/n";
	...
	do{
		newNode.createAnchor(pitchIteration, &curr, numNodes, curr.pathLength + 1, aspect, zNear, globalBB);
		cout<< "high level: " << highLevelCutOff << "\n";
		cout<< "newNode Weight: " << newNode.weight << "\n";
   }while(newNode.weight < highLevelCutOff); //delta(newNode.depth,curr.depth) > 100 && delta(newNode.camPhi,curr.camPhi) > 100  && 
   

	cout << "new node added to map: " << roadMap.size() << "\n";
   newNode.ndex = roadMap.size();
   //cout << "curr index: " << newNode.ndex << "\n";
   //cout << "curr Parents index: " << newNode.parentIndex << "\n";
